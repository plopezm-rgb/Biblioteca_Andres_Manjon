...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $usuario) {
    ...
    if (empty($nombre) || empty($username)) {
        ...
        try {
            ...
            header("Refresh:1; url=buscar_alumno.php?busqueda=" . urlencode($busqueda) . "&tipo_busqueda=" . urlencode($tipo_busqueda));
...
<body>
    ...
    <main>
        <section class="edit-container">
            <a href="buscar_alumno.php?busqueda=<?= urlencode($busqueda) ?>&tipo_busqueda=<?= urlencode($tipo_busqueda) ?>" class="btn-volver">← Volver a la búsqueda</a>
